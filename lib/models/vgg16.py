...
class VGG16(model.Model):
    ...
    def init(self):
        if self._kerasModel is True:
            self.__tryToLoadModelFromKeras()
        if self._localModel is True:
            self.__tryToLoadModelFromLocalVolume()
    ...
    def fineTune(self, fitSamples, valSamples, testSamples, fitLabels, valLabels, testLabels, trainDir):
        ...
        if self.__lookForPreviousExtractedFeatures(fineTunningFolder) is True:
            self._log.debug("{0} - fineTune - Found previous extracted features. Loading files from {1}".format(
                            self._log_prefix, fineTunningFolder))
            ...
            # Extract features
            self._log.debug("{0} - fineTune - Initiating feature extraction from conv_base ...".format(
                            self._log_prefix))
            ...
            os.makedirs(fineTunningFolder, exist_ok=True)
        ...
        fitFeatures = np.reshape(fitFeatures, (len(fitSamples), self._batch_size))
        valFeatures = np.reshape(valFeatures, (len(valSamples), self._batch_size))
        testFeatures = np.reshape(testFeatures, (len(testSamples), self._batch_size))
        # Compose new model
    ...
    def __addExtraLayers(self):
        ...
        model.add(layers.Dense(self._batch_size, activation='relu', input_dim=(self._batch_size)))
    ...
    def __tryToLoadModelFromKeras(self):
        if self._fineTunning is True:
            self._log.debug("{0} - __tryToLoadModelFromKeras - Instanciating conv_base for fine tunning".format(
                            self._log_prefix))
            inputShape = (self._input_height, self._input_width, self._input_depth)
            self._conv_base = getattr(self._kerasAppsModule, self._modelNameFromKeras)(include_top=False,
