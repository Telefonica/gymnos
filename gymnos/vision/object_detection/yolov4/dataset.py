...
def fill_truth_detection(bboxes, num_boxes, num_classes, flip, dx, dy, sx, sy, net_w, net_h):
    if bboxes.shape[0] == 0:
        return bboxes, 10000
    ...
    bboxes = bboxes[np.where((bboxes[:, 4] < num_classes) & (bboxes[:, 4] >= 0))[0]]
...
class YOLODataset(Dataset):
    ...
    def __init__(self, labels_path, dataset_dir, mixup, letter_box, width, height, jitter, hue,
                 saturation, exposure, flip, blur, gaussian, boxes, num_classes, train=True):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        for i in range(use_mixup + 1):
            ...
            truth, min_w_h = fill_truth_detection(bboxes, self.boxes, self.num_classes, flip, pleft, ptop, swidth,
                                                  sheight, self.width, self.height)
            if (min_w_h / 8) < blur and blur > 1:  # disable blur if one of the objects is too small
                blur = min_w_h / 8

            ai = image_data_augmentation(img, self.width, self.height, pleft, ptop, swidth, sheight, flip,
            ...
            if use_mixup == 1:
                ...
                left_shift = int(min(cut_x, max(0, (-int(pleft) * self.width / swidth))))
                top_shift = int(min(cut_y, max(0, (-int(ptop) * self.height / sheight))))

                right_shift = int(min((self.width - cut_x), max(0, (-int(pright) * self.width / swidth))))
                bot_shift = int(min(self.height - cut_y, max(0, (-int(pbot) * self.height / sheight))))

                out_img, out_bbox = blend_truth_mosaic(out_img, ai, truth.copy(), self.width, self.height, cut_x,
